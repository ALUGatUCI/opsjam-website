...
export async function POST(request: NextRequest) {
  ...
  logger.info('Mailing list signup request received', { email: maskEmail(trimmedEmail) })
  ...
  try {
    ...
    logger.error('Mailing list signup failed', { email: maskEmail(trimmedEmail), error: serializeError(error) })
